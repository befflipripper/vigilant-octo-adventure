...
window.onload = function () {
  ...
  function create() {
    ...
    text2 = game.add.text(200, 16, "LIVES: " + lives, { font: "bold 24px Arial", fill: "white" });
  ...
  function update() {
    ...
    if (cursors.left.isDown && !lose) {
      player.animations.play('walk', 10, true);
      player.body.velocity.x = -300;
      player.scale.x = - 1;
    }
    // is the right cursor key pressed?
    else if (cursors.right.isDown && !lose) {
      player.animations.play('walk', 10, true);
      player.body.velocity.x = 300;
      player.scale.x = 1;
    }
    ...
    if (jumpButton.isDown && (player.body.onFloor() || player.body.touching.down) && !lose) {
